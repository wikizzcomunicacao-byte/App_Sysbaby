...
st.set_page_config(page_title="Sys Baby Kids - Catálogo Digital", layout="wide", page_icon="🗄️")
...
# --- VERIFICA SE O CLIENTE ABRIU UM LINK DE ÁLBUM ESPECÍFICO (?produto=ID) ---
query_params = st.query_params
produto_compartilhado_id = query_params.get("produto")

if produto_compartilhado_id:
    # MODO CLIENTE: Exibe apenas o álbum limpo do produto compartilhado
    st.title("👶 Sys Baby Kids — Álbum Exclusivo")
    st.markdown("---")
    
    try:
        resp_prod = supabase.table("projetos_moveis").select("*").eq("id", produto_compartilhado_id).execute()
        prod_data = resp_prod.data if resp_prod.data else []
        
        if not prod_data:
            st.error("Ops! Este produto não foi encontrado ou o link é inválido.")
        else:
            item = prod_data[0]
            st.header(item.get("ambiente"))
            ...
            with col_d1:
                if lista_urls:
                    st.write(f"**Galeria de Fotos ({len(lista_urls)}):**")
                    for url in lista_urls:
                        st.image(url, use_container_width=True)
                else:
                    st.info("Nenhuma foto cadastrada para este item.")
            ...
            with col_d2:
                ...
                st.markdown(f"<span style='color: #2C5E3B; font-weight: bold; font-size: 1.4em;'>R$ {float(item.get('preco') or 0):,.2f}</span>", unsafe_allow_html=True)
                
                st.markdown("---")
                st.info("Gostou deste modelo? Entre em contato conosco para encomendar o seu projeto exclusivo!")
                
    except Exception as e:
        st.error(f"Erro ao carregar o álbum: {e}")

else:
    # MODO SISTEMA NORMAL (ADMIN / GERENCIAMENTO)
        ...
        try:
            ...
            if not itens:
                ...
                if st.session_state["item_selecionado_id"] is None:
                    ...
                    st.markdown("Clique em **'Ver Detalhes & Proposta'** em qualquer item para abrir a tela dedicada e gerar o link do álbum.")
                ...
                # --- TELA 2: DETALHES DO ITEM, LINK DO ÁLBUM E PDF ---
                else:
                    ...
                    with col_det2:
                        ...
                        # --- LINK PÚBLICO DO ÁLBUM ---
                        st.markdown("---")
                        st.subheader("🌐 Link Público do Álbum para o WhatsApp")
                        
                        # Monta o link automático do app apontando para este produto
                        base_url = "https://appsysbabykids.streamlit.app" # Substitua se necessário pelo seu link exato do Streamlit
                        link_album = f"{base_url}/?produto={item_atual.get('id')}"
                        
                        st.text_input("Copie o link abaixo para enviar ao cliente:", value=link_album)
                        
                        preco_item = float(item_atual.get('preco') or 0)
                        texto_zap_album = urllib.parse.quote(f"Olá! Segue o link com o álbum completo de fotos e detalhes do móvel *{item_atual.get('ambiente')}* da Sys Baby Kids. Valor: R$ {preco_item:,.2f}.\n\nVeja aqui: {link_album}")
                        link_whatsapp_album = f"https://wa.me/?text={texto_zap_album}"

                        st.markdown(
                            f"""
                            <a href="{link_whatsapp_album}" target="_blank" style="display:block;text-align:center;padding:10px 20px;background-color:#25D366;color:white;text-decoration:none;font-weight:bold;border-radius:6px;margin-top:10px;">
                                💬 Enviar Álbum Direto no WhatsApp
                            </a>
                            """,
                            unsafe_allow_html=True
                        )
                    ...
                    st.markdown("---")
                    st.markdown("### Geração de Proposta em PDF")
